...
# we can use psutil module instead of resource 'it works on windows too'
# import psutil
# time = time.process_time()
# mem = process.memory_info().rss

def file_reader_with_generator(input_file):
...
def main():
    try:
        if len(sys.argv) != 2:
            raise ValueError("Usage: python3 generator.py <input_file>")
        ...
        lines = file_reader_with_generator(file_path)
        for line in lines:
            pass
        ...
        peak_mem_usage = end_mem / (1024 * 1024)
